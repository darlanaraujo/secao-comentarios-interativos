// COMANDOS PARA PUXAR O CONTEÚDO DO JSON E ORGANIZAR NA PÁGINA ============================================
...
jsonDados.forEach((dados) => {
    ...
    comments.forEach((item) => {
        ...
        item.getElementsByClassName('js-content')[0].innerHTML = dados.comments[cont].content;
        item.setAttribute('id', `comment${cont +1}`);
        cont++;
    });
    cont = 0;

    replices.forEach((item) => {
        item.getElementsByClassName('js-image')[0].src = dados.comments[cont].replies[cont].user.image.png;
        item.getElementsByClassName('js-username')[0].innerHTML = dados.comments[cont].replies[cont].user.username;
        item.getElementsByClassName('js-createdAt')[0].innerHTML = dados.comments[cont].replies[cont].createdAt;
        item.getElementsByClassName('js-score')[0].innerHTML = dados.comments[cont].replies[cont].score;
        item.getElementsByClassName('js-replyngto')[0].innerHTML = `@${dados.comments[cont].replies[cont].replyingTo}`;
        item.getElementsByClassName('js-content')[0].innerHTML = dados.comments[cont].replies[cont].content;
        item.setAttribute('id', `replice${cont +1}`);
        cont++;
    });
    cont = 0;
});
...
const comment = document.querySelectorAll('[id^="comment"]');
const replice = document.querySelectorAll('[id^="replice"]');

replice.forEach((dados) => {
    // console.log(dados);
    // console.log(dados.getElementsByClassName('btn-mais'));
    dados.getElementsByClassName('btn-editar')[0].addEventListener('click', (event) => {
        console.log('Clicou!');
